...
# %% [ SOURCE DATA PLOT - MAIN FIGURE 2B ]
tick_fs = 8
BIN_SIZE = 10
MARGIN_MS = 150
MIN_BURST_DUR_MS = 180
MAX_BURST_DUR_MS = 800
df_all = pd.read_excel(sd_path, sheet_name="Fig2B", header=None)
...
fig = plt.figure(figsize=(2.25,2.25), dpi=300)
...
ylim_max = 40 #0.30
ybar_height = 10 # EMG magnitude (uV) 0.001 # 0.5 for PCs/EMG, 20spk/s for lfads
...
norm = matplotlib.colors.Normalize(vmin=MIN_BURST_DUR_MS, vmax=MAX_BURST_DUR_MS)
cbar = fig.colorbar(colormap.ScalarMappable(norm=norm, cmap=cm), ax=ax, orientation="horizontal", shrink=0.85, pad=0.03)
cbar.set_label(f"Extensor burst dur. (ms)", fontsize=fs-1)
cbar.ax.set_yticklabels(cbar.ax.get_yticks().astype(int), fontsize=fs-1)
